...
//! by editing `open_session` and timing `meethook transcribe` -- that loop takes minutes per
//! configuration and measures the wrong thing, because `transcribe` is dominated by Whisper on a
//! 1.6 GB checkpoint and would hide a few hundred milliseconds of model load entirely. So the
//! measurement lives here, where it costs seconds, and nothing on the `meethook transcribe` path
//! calls it.
//!
//! # The grid
//!
//! Four cells, not three: `NeuralNetwork` and `MLProgram`, each with and without a cache. The
//! fourth cell costs one more pass and is otherwise the one that gets assumed. Cell 1 --
//! `NeuralNetwork`, no cache -- is what ships today, so it is the baseline every later cell is
//! compared against.
//!
//! # How to read the numbers
//!
//! **Cold and warm load.** Each graph is loaded twice against the same cache directory. Without a
//! cache dir CoreML recompiles on every load, so a single load time cannot separate "compiled
//! again" from "this format is slow to compile". For the two uncached cells the two numbers
//! should come out equal, and that equality is itself the measurement that the cache is doing
//! something in the other two.
//!
//! **Nodes supported.** ONNX Runtime's own partitioning report, captured from its log rather than
//! guessed at: how many partitions it handed CoreML, how many nodes the graph has, and how many
//! of them CoreML took. This is the *why* behind the wall clock -- the premise that `MLProgram`
//! supports more operators, so fewer of pyannote's ops get partitioned back onto the CPU, is
//! exactly the thing this column is here to confirm or refute. A configuration that is faster
//! with worse coverage is a result worth looking at twice. ONNX Runtime calls `GetCapability`
//! more than once per session (partitioning, then again after layout transformation), so every
//! round is printed in order and the last is the one that describes what actually ran.
//!
//! **Both graphs separately.** 6 MB of pyannote segmentation against 26 MB of ResNet34 embedder;
//! a format that helps one and hurts the other is exactly the outcome a single total would hide.
//!
//! **A turn divergence is expected, not alarming.** CoreML runs fp16 on the ANE and the two model
//! formats are under no obligation to agree to the last bit, so a divergence is reported -- which
//! turn, and what each side said -- rather than asserted against. Cell 1 runs its pass twice on
//! the same sessions as a control: if pass A and pass B differ, the whole divergence column is
//! describing run-to-run noise and not the configuration, and the report says so.
//!
//! # Two caveats before believing a gap
//!
//! A warm load *in the same process* is a proxy for a second process, not the thing itself. The
//! real measurement is `--cache-dir <path>`, run twice: the second run's cold column is what a
//! genuine second process pays. Without that flag every cache directory is a temporary one, so a
//! run neither reads a cache an earlier run left nor leaves one behind.
//!
//! And these are single-digit-second numbers on a laptop that thermally throttles. Run the tool
//! more than once, on a quiet machine, before believing a difference.
