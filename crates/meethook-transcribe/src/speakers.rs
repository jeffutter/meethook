...
use crate::voice_vectors::{group_distance, group_mean, normalize};
...
///
/// Public so that a diagnostic reporting [`crate::GroupDistance::average_linkage`] can say
...
/// side, an above-floor cluster's on the other -- [`crate::GroupDistance::centroid`], 0 for
/// the same direction and 1 for orthogonal. Not turn-to-turn, and *not*
/// [`crate::GroupDistance::average_linkage`],
...
/// nobody. The arithmetic is [`group_mean`]'s; this only maps member indices to the borrowed
/// slices it takes and drops the length that step also returns.
fn reference_embedding(members: &[usize], embeddings: &[Vec<f32>]) -> Vec<f32> {
    let vectors: Vec<&[f32]> = members.iter().map(|&i| embeddings[i].as_slice()).collect();
    group_mean(&vectors)
        .map(|(unit, _)| unit)
        .expect("a cluster's reference needs members with a non-zero mean direction")
}
...
/// same quantity as [`crate::GroupDistance::average_linkage`], which is computed from the
/// embeddings with no constraint in it: one is a merge decision, the other is a distance.
...
mod tests {
    ...
    use crate::voice_vectors::{at, shapes, two_clouds, vectors};
    ...
    /// The claim that makes reporting [`crate::GroupDistance::average_linkage`] honest: it is
    /// the same
